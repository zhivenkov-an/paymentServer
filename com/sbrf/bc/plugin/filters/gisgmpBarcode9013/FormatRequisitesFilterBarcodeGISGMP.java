...
public class FormatRequisitesFilterBarcodeGISGMP implements PaymentsFilterFactory {

    private final Config config;

    public FormatRequisitesFilterBarcodeGISGMP(Properties properties) {
    ...
    public BrakValue[] check(PaymentData paymentData, BrakValue[] brakArray) throws FilterException {
     ...
     // если код.спецклиента и тип платежа равны заданным в конфиге, то добавляем ИУН    
        ...
        if (paymentData != null && paymentData.getSpecialClientCode() != null && paymentData.getSpecialClientCode().equals(config.formatSpecCode1) && (paymentData.getPaymentType() == 3 || paymentData.getPaymentType() == 4)) {
                ...
                if (addReq.length <= (config.filNumSeriya-1)){ //если не хватает дополнительного реквизита серия и номер постановления
                    ...
                    // специально для Штрих-кода Воронежского ГИБДД
                    if ( seriyaPost.length()>17 ) {
                        seriyaPost =  seriyaPost.substring(1, 3) + verificator.getCharByCode(Integer.parseInt(seriyaPost.substring(3, 7))) + verificator.getCharByCode(Integer.parseInt(seriyaPost.substring(7, 11))) + seriyaPost.substring(11,17);                        
                    }
                ...
                calDay.set(Calendar.MONTH,Integer.parseInt(datePost.substring(2,4))-1); // т.к. отсчет месяца идёт с нуля вычитаем единицу
             ...
             // преобразуем в модифицированный base64                
                ...
                do{
                  ...
                  // tempSt = tempSt + "Останок " + ost + " закодирован " + base64CrcRusChars.charAt(ost);
                } while (des != 0);
                String stUIN;
    ...
    Config(PluginConfigurationHelper context) {                  
        formatSpecCode1 = context.getString("formatSpecCode1", "07361");
