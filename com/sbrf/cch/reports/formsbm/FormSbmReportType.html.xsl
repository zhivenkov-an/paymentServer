...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/root">
        <html xml:lang="ru">
            ...
            <body>
                <xsl:choose>
                    <xsl:when
                        test="count(/root/items/bik) + count(/root/items/osb) + count(/root/items/settle_acc) + count(/root/items/payment_date) + count(/root/items/transfer_date) + count(/root/items/special_client_code) + count(/root/items/total_summary) = 0">
        ...
        <xsl:choose>
            <xsl:when test="count(bik) != 0">
                <xsl:apply-templates select="bik">
                    ...
                    <xsl:with-param name="osbCount"
                        select="$osbCount" />
                    <xsl:with-param name="paymentCount"
                        select="$paymentCount" />
                    <xsl:with-param name="transferCount"
                        select="$transferCount" />
            ...
            <xsl:when test="$transferCount != 0">
                <xsl:apply-templates select="transfer_date">
                    ...
                    <xsl:with-param name="transferCount"
                        select="$transferCount" />
                    <xsl:with-param name="speccode"
                        select="''" />
                    <xsl:with-param name="osb" select="''" />
                    <xsl:with-param name="payment_date"
                        select="''" />
            ...
            <xsl:when test="count(total_summary) != 0">
                ...
                <table border="1" cellspacing="0" style="text-align:right; table-layout:fixed; width:41em;" cellpadding="3">
                    <tr>
                    ...
                    <xsl:apply-templates select="total_summary">
                        <xsl:with-param name="speccode"
                            select="''" />
                        <xsl:with-param name="osb"
                            select="''" />
                        <xsl:with-param name="payment_date"
                            select="''" />
                        <xsl:with-param name="transfer_date"
                            select="''" />
                        <xsl:with-param name="specCount"
                            select="$specCount" />
                        <xsl:with-param name="osbCount"
        ...
        <xsl:choose>
            <xsl:when test="0 = count(settle_acc)">
                ...
                <table border="1" cellspacing="0"
                    style="text-align:right; table-layout:fixed;"
                    cellpadding="3">
                    <thead>
                        <tr>
    ...
    <xsl:template match="osb">
        ...
        <xsl:choose>
            <xsl:when
                test="($bik = '') and ($settleAcc = '') and ($specCount = 0)">
                <tr>
                <td>
                <h4 style="margin-bottom: 3px;">
    ...
    <xsl:template match="transfer_date">
        ...
        <xsl:choose>
            <xsl:when
                test="($bik = '') and ($settleAcc = '') and ($specCount = 0) and ($osbCount = 0) and ($paymentCount = 0)">
                <tr>
                <td>
                <h4 style="margin-bottom: 3px;">
    ...
    <xsl:template match="summary">
        ...
        <tr>
            ...
            <xsl:choose>
                <xsl:when
                    test="($paymentCount &lt; 0) or ($paymentCount = 0)">
                </xsl:when>
                <xsl:otherwise>
                    <td
                        style="border-top-width:0px; border-left-width:0px; border-right-width:0px; text-align:center;">
            ...
            <xsl:choose>
                <xsl:when
                    test="($transferCount &lt; 0) or ($transferCount = 0)">
                </xsl:when>
                <xsl:otherwise>
                    <td
                        style="border-top-width:0px; border-left-width:0px; border-right-width:0px; text-align:center;">
