...
public class TestReport implements ReportType, BillingPlugin{
    ConnectionSource co;
    
    public TestReport(Properties properties) {
    ...
    public String[] getFormats() {
        ...
        return new String[]{ReportFormats.XML};
    }
    ...
    public Parameter[] getParameters() throws ReportException {
        ...
        Parameter bik = new InputParameter("bik", "БИК").setMinLength(9).setMaxLength(9).setDefault("jvbjhbji").setRequired(true);
    ...
    public void prepareData(Map inParameters, Map arg1, OutputStream out) throws InterruptedException, ReportException {
        ...
        ResultSet rs = null; 
        try{
        out.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?> <root><items>".getBytes()); // TODO encoding
        ...
        PreparedStatement preparedStatement = connection.prepareStatement("select sum, inn from pay.payments where transfer_date = ? and bik = ? ");
                ...
                while(rs.next()) {
                    ...
                    out.write(("<item sum='" + q + "' inn='" + inn + "'/>").getBytes());
                }
                out.write("</items></root>".getBytes());
